...
Linchpin.Centrality <- function(links = edgelist, characteristics = key, label_column_name = char, weighted = FALSE) {
     ...
     key$ID_1 <- as.character(key$ID_1)
     links.results[[3]] <- key <- key %>% left_join((lookup_df %>% select(ID_1, linkage)))
